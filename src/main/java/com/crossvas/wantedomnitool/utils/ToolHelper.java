...
public class ToolHelper {
    ...
    public static boolean shearEntity(ItemStack stack, EntityPlayer player, EntityLivingBase entity) { // stolen from
        // ItemShears
    ...
    public static boolean shearBlock(ItemStack stack, BlockPos pos, EntityPlayer player) { // stolen from ItemShears
        ...
        if (block instanceof IShearable) {
            ...
            if (target.isShearable(stack, player.world, pos)) {
                ...
                for (ItemStack drop : drops) {
                    float f = 0.7F;
                    double x = player.world.rand.nextFloat() * f + (1.0F - f) * 0.5D;
                    double y = player.world.rand.nextFloat() * f + (1.0F - f) * 0.5D;
                    double z = player.world.rand.nextFloat() * f + (1.0F - f) * 0.5D;
                    EntityItem entityItem = new EntityItem(player.world, pos.getX() + x, pos.getY() + y,
                            pos.getZ() + z, drop);
    ...
    public static boolean canAoEHarvest(World world, IBlockState state, BlockPos pos, BlockPos originPos,
                                        EntityPlayer player) {
    ...
    // Stolen from Tinker's Construct
    ...
    public static void breakBlock(ItemStack stack, World world, IBlockState state, BlockPos pos, EntityPlayer player) {
        ...
        if (!world.isRemote) { // server sided handling
            ...
            mc.getConnection().sendPacket(new CPacketPlayerDigging(CPacketPlayerDigging.Action.STOP_DESTROY_BLOCK, pos,
